 ...
 *   live mode  : FastAPI backend at /api/... (local dev with Vite proxy) or remote Render URL  
 ...
 * Set VITE_API_BASE_URL to use a remote API (e.g., Render deployment)  
 */  
  
const STATIC   = import.meta.env.VITE_STATIC_DATA === 'true'  
const BASE_URL = import.meta.env.BASE_URL || '/'          // e.g. /-Live-Bench/  
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || '' // e.g., https://clawwork-earnbot-deepseek.onrender.com  
  
const staticUrl = (path) => `${BASE_URL}data/${path}`  
const liveUrl   = (path) => API_BASE_URL ? `${API_BASE_URL}/api/${path}` : `/api/${path}`  
...
export const getArtifactFileUrl = (path) =>  
  STATIC  
    ...
    : (API_BASE_URL ? `${API_BASE_URL}/api/artifacts/file?path=${encodeURIComponent(path)}` : `/api/artifacts/file?path=${encodeURIComponent(path)}`)  
...
export const saveHiddenAgents = (hiddenArray) => {  
  ...
  return fetch(`${apiUrl}/api/settings/hidden-agents`, {  
    method: 'PUT',  
